...
**Status: NICHT IMPLEMENTIERT — Design entschieden (Best Practice): v1 rein
verhaltensbasiert (kein Schema-Deklarations-Advisory); Crash-Erkennung via
Liveness-Recheck ohne Transport-Umbau. Bereit für Phase 1 auf Signal.**
...
- [ ] **T003** `core/models.py`: `FuzzProbeClass` (`TYPE_MISMATCH`, `OVERSIZED`,
...
- [ ] **T004** `tests/fixtures/fuzzing_servers.py`: HTTP-Fixture, Modi
...
- [ ] **T005** `tests/fixtures/stdio_server.py`: `fuzz`-Modus (Crash = Prozess-Tod
  bei Payload; clean = strukturierter Fehler) für stdio-Transport-Parität.
- [ ] **T006** `tests/test_schema_fuzzing.py` (rot): US1 (Crash, HTTP+stdio),
...
- [ ] **T007** `checks/schema_fuzzing.py`: `tools/list` lesen, lesende Tools
...
- [ ] **T009** Fehlerantworten auf konservative Leak-Marker prüfen (Traceback/
