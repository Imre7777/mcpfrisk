...
12 statische Checks sind implementiert, getestet und funktionieren:
...
| `PACKAGE_PROVENANCE` | `checks/package_provenance.py` | Wrapper um `npm audit signatures` (analog DEPENDENCY_SCAN, DI-Runner): npm-Dependency mit ungültiger (HIGH) / fehlender (LOW) Registry-Signatur. Fehlende Provenance-Attestation wird bewusst NICHT geflaggt (FP). npm+Lockfile nötig, sonst skipped; Tool-Fehler → INFO; `run()` wirft nie. Feature `021-package-provenance` | MCP04 | CWE-347 |

Alle zwölf sind in `mcpfrisk/checks/registry.py` registriert. **`MCP_CONFIG_AUDIT`
...
│   ├── package_provenance.py  # Tier 1: PACKAGE_PROVENANCE (npm-Signatur-Wrapper, Feature 021)
